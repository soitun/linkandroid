...
const start = async (): Promise<number> => {
    ...
    try {
        ...
        wss.on("connection", (ws: WebSocket, req) => {
                ...
                return {
                    ...
                    data: {
                        buttons: [
                            ...
                            {
                                id: "follow",
                                icon: isFollowMode ? "follow_active" : "follow",
                            },
                            {
                                id: "toggle_top",
                                icon: isTopMode ? "top_active" : "top",
                            },
                            { id: "close", icon: "quit" },
                        ],
                    },
            ...
            Log.info(
                `WebSocket client connected: type=${clientType}, deviceId=${deviceId}`,
            );
                ...
                try {
                    ...
                    if (clientType === "DeviceManage") {
                        ...
                        if (data.type === "ready") {
                            ...
                            Log.info(
                                `Sent panel config to DeviceMirror: deviceId=${deviceId}`,
                            );
                        } else if (data.type === "panel_button_click") {
                            const buttonId = data.data?.id;
                            if (buttonId === "follow") {
                                const currentState =
                                    deviceFollowMode.get(deviceId) || false;
                                deviceFollowMode.set(deviceId, !currentState);
                                Log.info(
                                    `FollowMode toggled for ${deviceId}: ${!currentState}`,
                                );
                                ws.send(
                                    JSON.stringify(getPanelConfig(deviceId)),
                                );
                            } else if (buttonId === "toggle_top") {
                                const currentState =
                                    deviceTopMode.get(deviceId) || false;
                                const newState = !currentState;
                                deviceTopMode.set(deviceId, newState);
                                Log.info(
                                    `TopMode toggled for ${deviceId}: ${newState}`,
                                );
                                ws.send(
                                    JSON.stringify({
                                        type: "top",
                                ...
                                if (
                                    isFollowMode &&
                                    isPanelButtonFollowable(buttonId)
                                ) {
                                    handlePanelButtonFollowMode(
                                        deviceId,
                                        buttonId,
                                    );
                                }
                    ...
                    Log.error(
                        `Failed to process message from ${clientType} (${deviceId}): ` +
                            error.message,
                    );
                }
            });
            ...
            ws.on("close", () => {
                ...
                Log.info(
                    `WebSocket client disconnected: type=${clientType}, id=${deviceId}`,
                );
                if (clientType === "DeviceManage") {
...
const handlePanelButtonFollowMode = (
    ...
    Log.info(
        `FollowMode: Broadcasting panel button from ${sourceDeviceId}`,
        buttonId,
    );
    let targetCount = 0;
    wss?.clients.forEach((client) => {
        if (client.readyState === WebSocket.OPEN) {
            ...
            if (
                clientType === "DeviceManage" &&
                clientDeviceId &&
                clientDeviceId !== sourceDeviceId
            ) {
                try {
                    // 发送按下和抬起两个事件
                    ...
                    targetCount++;
                    Log.info(
                        `FollowMode: Panel button sent to ${clientType}:${clientDeviceId} ${buttonId}->${keyCode}`,
                    );
                } catch (error) {
                    Log.error(
                        `FollowMode: Failed to send panel button to ${clientDeviceId}: ${error.message}`,
                    );
                }
            }
        }
    ...
    if (targetCount === 0) {
        Log.info(
            `FollowMode: No target devices to forward panel button (source: ${sourceDeviceId})`,
        );
    }
};
...
const handleFollowMode = (sourceDeviceId: string, eventData: any) => {
    Log.info(
        `FollowMode: Broadcasting event from ${sourceDeviceId}`,
        eventData.type,
    );
    let targetCount = 0;
    wss?.clients.forEach((client) => {
        if (client.readyState === WebSocket.OPEN) {
            ...
            if (
                ...
                try {
                    ...
                    targetCount++;
                    Log.info(
                        `FollowMode: Event sent to ${clientType}:${clientDeviceId} ${JSON.stringify(eventData)}`,
                    );
                } catch (error) {
                    Log.error(
                        `FollowMode: Failed to send event to ${clientDeviceId}: ${error.message}`,
                    );
                }
            }
        }
    ...
    if (targetCount === 0) {
        Log.info(
            `FollowMode: No target devices to forward (source: ${sourceDeviceId})`,
        );
    }
};
                ...
                clientSet.forEach((ws) => {
                    if (ws.readyState === WebSocket.OPEN) {
                        try {
                            ...
                            Log.error(
                                `Failed to send quit command to ${clientType}: ${error.message}`,
                            );
                        }
                    }
                });
