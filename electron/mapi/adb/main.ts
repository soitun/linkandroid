...
class DeviceScanner {
    ...
    startScanning(type: string, callback: (device: DeviceData) => void) {
        ...
        console.log(
            `[Main] DeviceScanner.startScanning 开始查找服务类型: ${type}`,
        );
        this.scanner = this.bonjour.find({ type }, (service: any) => {
            console.log(
                "[Main] DeviceScanner 发现服务:",
                JSON.stringify({
                    name: service.name,
                    type: service.type,
            ...
            if (device) {
                ...
                console.log(
                    "[Main] DeviceScanner 解析设备失败 (可能没有 IPv4 地址)",
                );
            }
        });
...
class AdbScanner {
        ...
        try {
            if (this.window && !this.window.isDestroyed() && this.callbackId) {
                ...
                if (
                    ...
                    webContents.send(`adb-scanner-status-${this.callbackId}`, {
                        status,
                        error,
                    });
    ...
    private async scanForDevice(): Promise<DeviceData> {
        ...
        const allServicesScanner = debugBonjour.find(
            { type: "" },
            (service: any) => {
                if (
                    service.type &&
                    (service.type.includes("adb") ||
                        service.type.includes("ADB"))
                ) {
                    console.log(
                        "[Main] 发现 ADB 相关服务:",
    ...
    private async pairWithDevice(device: DeviceData, password: string) {
        try {
            ...
            console.log(
                `[Main] 执行配对命令: ${adbPath} pair ${device.address}:${device.port}`,
            );
            ...
            return new Promise<void>((resolve, reject) => {
                ...
                process.on("close", (code) => {
                    ...
                    if (
                        stdout.includes("Successfully paired") ||
                        stderr.includes("Successfully paired")
                    ) {
                        resolve();
                    } else {
                        reject(
                            new Error(
                                `Pairing failed with code ${code}: ${stderr || stdout}`,
                            ),
                        );
                    }
                });
    ...
    private async waitForDeviceConnect(
        device: DeviceData,
    ): Promise<DeviceData> {
    ...
    private async connectToDevice(device: DeviceData) {
        try {
            const adbPath = await extraResolveBin("scrcpy/adb");
            console.log(
                `[Main] 执行连接命令: ${adbPath} connect ${device.address}:${device.port}`,
            );
            ...
            return new Promise<void>((resolve, reject) => {
                ...
                process.on("close", (code) => {
                    ...
                    if (code === 0 || stdout.includes("connected")) {
                        ...
                        reject(
                            new Error(`Connection failed: ${stderr || stdout}`),
                        );
                    }
                });
...
ipcMain.handle(
    "adb:scannerConnect",
    async (event, password: string, callbackId: string) => {
        console.log(
            "[Main] 收到 scannerConnect 请求, password:",
            password,
            "callbackId:",
            callbackId,
        );
