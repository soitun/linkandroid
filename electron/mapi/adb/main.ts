...
class AdbScanner {
        ...
        return new Promise((resolve, reject) => {
            ...
            const timeoutHandle = setTimeout(() => {
                if (!deviceFound) {
                    ...
                    reject(new MonitorError(ERROR_CODES.TIMEOUT, 'Connection attempt timed out'))
                }
            ...
            // 主扫描器
            this.deviceScanner.startScanning(MDNS_CONFIG.PAIRING_TYPE, handleDeviceFound)

            // 备用扫描器（使用标准格式）
            const altScanner = new DeviceScanner()
            altScanner.startScanning(MDNS_CONFIG.PAIRING_TYPE_ALT, handleDeviceFound)
        })
    }
    ...
    private async pairWithDevice(device: DeviceData, password: string) {
        try {
            ...
            throw new MonitorError(ERROR_CODES.PAIRING_FAILED, 'Unable to pair with device')
        }
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            const timeoutHandle = setTimeout(() => {
                if (!deviceFound) {
                    ...
                    scanner.dispose()
                    altScanner.dispose()
                    reject(new MonitorError(ERROR_CODES.TIMEOUT, 'Device connect timeout'))
                }
    ...
    private async connectToDevice(device: DeviceData) {
        try {
            ...
            throw new MonitorError(ERROR_CODES.CONNECTION_FAILED, `Failed to connect to device: ${error.message}`)
        }
    }
