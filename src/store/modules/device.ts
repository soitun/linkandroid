...
const createDeviceStatus = (
    record: DeviceRecord,
): ComputedRef<EnumDeviceStatus> => {
    const id = record.id;
    return computed(() => {
        return (
            deviceRuntime.value?.get(id)?.status ||
            EnumDeviceStatus.WAIT_CONNECTING
        );
    });
};
        ...
        ws.onmessage = (event) => {
            ...
            try {
                ...
                if (data.type === "DevicePreview") {
                    ...
                    const device = deviceStore().records.find(
                        (r) => r.id === deviceId,
                    );
...
const handlePanelButtonClick = async (deviceId: string, buttonId: string) => {
    console.log(
        `Handling panel button click: device=${deviceId}, button=${buttonId}`,
    );
...
const startDeviceManage = async (deviceId: string) => {
    try {
        ...
        const controller = await $mapi.scrcpy.spawnShell(
            ...
            {
                stdout: (data: string) => {
                    window.$mapi.log.info("Render.DeviceManage.stdout", {
                        deviceId,
                        data,
                    });
                },
                stderr: (data: string) => {
                    window.$mapi.log.error("Render.DeviceManage.stderr", {
                        deviceId,
                        data,
                    });
                },
                success: () => {
                    window.$mapi.log.info("Render.DeviceManage.success", {
                        deviceId,
                    });
                    deviceControllers.delete(deviceId);
                },
                error: (msg: string, exitCode: number) => {
                    window.$mapi.log.error("Render.DeviceManage.error", {
                        deviceId,
                        msg,
                        exitCode,
                    });
                    ...
                    setTimeout(() => {
                        ...
                        if (
                            device &&
                            device.status === EnumDeviceStatus.CONNECTED
                        ) {
        ...
        window.$mapi.log.error("Failed to start debug_manage:", {
            deviceId,
            error,
        });
    }
...
export const deviceStore = defineStore("device", {
    ...
    actions: {
        ...
        async doMirror(device: DeviceRecord) {
            ...
            const setting = {
                dimWhenMirror: await this.settingGet(
                    device,
                    "dimWhenMirror",
                    "no",
                ),
                ...
                videoBitRate: await this.settingGet(
                    device,
                    "videoBitRate",
                    "8M",
                ),
            ...
            try {
                runtime.value.mirrorController = await $mapi.scrcpy.mirror(
                    device.id,
                    {
        ...
        async settingGet(
            device: DeviceRecord,
            name: keyof DeviceSetting,
            defaultValue: string,
        ) {
            if (device.setting && name in device.setting) {
                if (
                    "" !== device.setting[name] &&
                    undefined !== device.setting[name]
                ) {
