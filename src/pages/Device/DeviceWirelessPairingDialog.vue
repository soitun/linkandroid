...
const startPairingService = async () => {
    try {
        ...
        console.log("Start scanning device...", password);

        try {
            const result = await window.$mapi.adb.scannerConnect(
                password,
                (status: string, error?: string) => {
                    ...
                    if (status === "pairing") {
                        ...
                        statusMessage.value =
                            error || t("device.pairingFailedShort");
                    }
                },
            );
            ...
            if (result.success) {
                ...
                Dialog.tipError(
                    t("device.pairingFailedWithError", { error: result.error }),
                );
            }
        } catch (connectError) {
            console.error("scannerConnect call error:", connectError);
            Dialog.tipError(
                t("device.pairingProcessError", { error: connectError }),
            );
            pairingStatus.value = "error";
        ...
        <div style="max-height: calc(100vh - 15rem)" class="-mx-2 -my-4">
            <div v-if="qrcodeUrl && pairingCode" class="flex gap-2">
                ...
                <div class="left-panel flex-shrink-0" style="width: 25rem">
                    ...
                    <!-- QR Code -->
                    <div
                        class="qrcode-container bg-white p-4 rounded-lg mb-3 text-center"
                    >
                        <img
                            :src="qrcodeUrl"
                    ...
                    <!-- Pairing Code -->
                    <div
                        class="pairing-info space-y-3 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg"
                    >
                        <div class="text-center">
                            <div
                                class="text-xl font-bold font-mono text-blue-600 dark:text-blue-400 tracking-wider"
                            >
                                {{ pairingCode }}
                            </div>
                            ...
                            <div class="flex items-center gap-2 text-sm">
                                <icon-loading
                                    v-if="
                                        ...
                                        pairingStatus === 'connecting-fallback'
                                    "
                    ...
                    <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                        <div class="text-sm text-blue-800 dark:text-blue-200">
                            ...
                            >
                                <li>
                                    {{
                                        $t("device.qrcodePairingInstructions1")
                                    }}
                                </li>
                                <li>
                                    {{
                                        $t("device.qrcodePairingInstructions2")
                                    }}
                                </li>
                                <li>
                                    {{
                                        $t("device.qrcodePairingInstructions3")
                                    }}
                                </li>
                                <li>
                                    {{
                                        $t("device.qrcodePairingInstructions4")
                                    }}
                                </li>
                                <li>
                                    {{
                                        $t("device.qrcodePairingInstructions5")
                                    }}
                                </li>
                            </ol>
                        </div>
