...
const doRecordProcess = async () => {
    ...
    await $mapi.adb.filePull(
        device.value.id,
        recordData.value.devicePath as string,
        localTempMp4Path,
    );
    await $mapi.adb.fileDelete(
        device.value.id,
        recordData.value.devicePath as string,
    );
    recordData.value.localTempMp4Path = localTempMp4Path;
    const resultPath =
        recordData.value.localTempDir + "/result." + recordData.value.format;
    try {
...
const doRecordDownload = async () => {
    ...
    let fromPath = recordData.value.resultPath[
        recordData.value.format
    ] as string;
    ...
    durationRefreshTimer = setInterval(() => {
        if (recordData.value.status === "recording") {
            ...
            recordData.value.duration = Math.floor(
                (recordData.value.endTimeInMs -
                    recordData.value.startTimeInMs) /
                    1000,
            );
        }
    }, 1000);
    ...
    >
        ...
        <div>
            <div class="px-2 relative">
                <a-form
                    v-if="recordData.status === 'idle'"
                    :model="recordData"
                    layout="vertical"
                >
                    <a-form-item>
                        <a-radio-group
                            v-model="recordData.format"
                            type="button"
                        >
                ...
                <a-form v-else :model="recordData" layout="vertical">
                        ...
                        <div class="flex">
                            <div class="mr-2">
                                ...
                                <a-tag
                                    color="red"
                                    size="large"
                                    v-else-if="recordData.status === 'fail'"
                                >
                                ...
                                <a-tag
                                    color="green"
                                    size="large"
                                    v-else-if="recordData.status === 'done'"
                                >
                    ...
                    <a-form-item>
                        ...
                        <a-button
                            v-if="
                                ['converting', 'fail', 'done'].includes(
                                    recordData.status,
                                )
                            "
                            :disabled="
                                ['converting', 'fail'].includes(
                                    recordData.status,
                                )
                            "
                            type="primary"
