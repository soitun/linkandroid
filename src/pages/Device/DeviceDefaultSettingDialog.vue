...
const show = async () => {
    formData.value.dimWhenMirror = setting.configGet(
        "Device.dimWhenMirror",
        "no",
    ).value;
    formData.value.alwaysTop = setting.configGet(
        "Device.alwaysTop",
        "no",
    ).value;
    formData.value.mirrorSound = setting.configGet(
        "Device.mirrorSound",
        "no",
    ).value;
    formData.value.previewImage = setting.configGet(
        "Device.previewImage",
        "no",
    ).value;
    visible.value = true;
...
const doSubmit = async () => {
    await setting.setConfig(
        "Device.dimWhenMirror",
        formData.value.dimWhenMirror,
    );
        ...
        <div v-if="visible" class="-mx-5 -my-6">
            <div class="overflow-y-auto p-5" style="height: calc(80vh - 200px)">
                <div>
                    ...
                    <div class="flex mb-3">
                        <div class="flex-grow">
                            {{ $t("device.dimWhenMirror") }}
                        </div>
                        <div class="">
                            <SettingItemYesNo
                                v-model="formData.dimWhenMirror"
                            />
                        </div>
                    </div>
                    <div class="flex mb-3">
                    ...
                    <div class="flex mb-3">
                        ...
                        <div class="">
                            <a-input
                                :model-value="
                                    setting.configGet('Device.maxFps', '60')
                                        .value
                                "
                                style="width: 10rem"
                                size="small"
                    ...
                    <div class="flex mb-3">
                        ...
                        <div class="">
                            <a-input
                                :model-value="
                                    setting.configGet('Device.scrcpyArgs', '')
                                        .value
                                "
                                style="width: 10rem"
                                size="small"
