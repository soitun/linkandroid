...
const doRefresh = async () => {
    Dialog.loadingOn();
    const files = await window.$mapi.adb.fileList(
        device.value.id,
        filePath.value || "/",
    );
    Dialog.loadingOff();
...
const doDelete = async () => {
    ...
    for (let f of files) {
        await window.$mapi.adb.fileDelete(
            device.value.id,
            filePath.value + "/" + f.name,
        );
    }
    Dialog.loadingOff();
...
const doUpload = async () => {
    ...
    if (path) {
        ...
        const fileName = window.$mapi.app.isPlatform("win")
            ? path.split("\\").pop()
            : path.split("/").pop();
    ...
    if (path) {
        ...
        for (let f of files) {
            ...
            if (f.isDirectory) {
                ...
                await window.$mapi.adb.filePull(
                    device.value.id,
                    sourcePath,
                    targetPath,
                );
...
const toggleSortByModifiedTime = () => {
    ...
    sortOrderModifiedTime.value =
        sortOrderModifiedTime.value === "asc" ? "desc" : "asc";
};
</script>
    ...
    >
        ...
        <div style="height: 60vh; margin: -0.5rem" class="">
            <div class="flex flex-col h-full">
                ...
                <div
                    class="flex-grow overflow-auto border border-solid border-gray-200 rounded p-2"
                >
                    ...
                    <div v-else class="flex flex-wrap">
                        ...
                        >
                            <div
                                class="border border-solid border-gray-200 rounded-lg mb-2 p-2 relative"
                            >
                                <div
                                    class="text-center p-3 cursor-pointer"
                                    @click="doOpen(f)"
                                >
                                    <FileExt
                                        :is-folder="f.isDirectory"
